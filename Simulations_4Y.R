...
for (zz in 1:3) {
  ...
  for (ii in 1:length(n)) {
    ...
    R = 20 # restart
    nregs = 2 #number of regressors
    multi_dim = 4 #number of Y
    ...
    if(model == "expectile"){
      save(results, file = paste0("c",wcop,"exp",n[ii],".",dist[zz],"4Y.RData"))
    } else if(model == "quantile"){
      save(results, file = paste0("c",wcop,"quant",n[ii],".",dist[zz],"4Y.RData"))
    }
